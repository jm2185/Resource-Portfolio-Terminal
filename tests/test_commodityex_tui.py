...
class DeliverErrorTest(unittest.TestCase):
    """A background ask that fails (timeout / CLI-missing / exception) must NOT leave the chat hung
    on 'thinking…'. `_deliver_error` clears `_pending_user`, drops the Working-lane entry + live
    preview, and surfaces the reason as a reply node in the thread."""

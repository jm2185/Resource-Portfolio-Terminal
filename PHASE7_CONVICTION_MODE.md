...
Weights and the V lens are **archetype-aware** (`v5_config.json → conviction_mode.pillar_weights_by_archetype`
/ `v_mode_by_archetype` — authoritative; live values below):

| Archetype | `w_T` | `w_Q` | `w_V` | V lens | Rationale |
|-----------|------:|------:|------:|--------|-----------|
| `option_convexity` (the spear) | 0.33 | 0.22 | 0.45 | **asymmetry** (bull-vs-REP-floor payoff + floor support) | Macro-driven convexity + asymmetry dominate; survival is a gate. |
| `commodity_cyclical` | 0.30 | 0.35 | 0.35 | value | Balanced — cash flows shift weight toward quality. |
| `asset_light_yield` (royalties) | 0.15 | 0.55 | 0.30 | value | Cash-flow quality dominates; macro lean is small. |
| `pure_macro_delta` | 0.45 | 0.25 | 0.30 | value | The macro *is* the thesis. |
| `_default` | 0.25 | 0.30 | 0.45 | value | Fallback. |

The **value** lens is fair-value-centred (`0.5 + 0.5·tanh((fair/price−1)/0.40)` blended with floor
support and a cash-flow-stability term), so a quality royalty at fair value reads mid-range with
value-investor directives (`QUALITY — CORE HOLD`, `FAIR VALUE — HOLD`) and quality-scale band labels,
never explorer "trim" calls.

**Conviction lift (non-linear aggregation).** A weighted average is inherently conservative, so a
bounded lift is added toward the *standout* pillar, scaled by how **earned** it is
(`conviction_lift.strength = 0.65`): `A = A_raw + s·c·max(0, anchor − A_raw)` with
`anchor = V, c = floor support` in asymmetry mode and `anchor = max(Q, V), c = JSF/4` in value mode.
It never exceeds the anchor pillar and is applied **before** the floor-aware gate.
...
## 5–7, 9–10. Implementation & tuning logs — archived

The shipped-status tables, review-pass refinements, layout-polish notes, and the two rating
tuning rounds were dated implementation history, not spec; they now live in
`docs/archive/PHASE7_IMPLEMENTATION_LOG.md`. The *operative* results of those rounds —
per-archetype pillar weights, the two V lenses, the conviction lift, and mode-aware band
labels — are folded into §2.5 above; `v5_config.json → conviction_mode` is authoritative.
...
(Implementation and tuning history: `docs/archive/PHASE7_IMPLEMENTATION_LOG.md`.)